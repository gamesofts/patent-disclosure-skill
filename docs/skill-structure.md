...
- **`agents/openai.yaml`**：Codex 产品形态元数据，提供展示名、短描述与默认调用提示。
...
| `agents/openai.yaml` | Codex 展示名、短描述、默认调用提示 |
| `prompts/` | 分步模板（录入、概念设计、可选扫描、专利点、查新、预览、成文、自检、迭代） |
...
在 Codex 中通常按 skill 根目录解析相对路径。兼容 Claude Code、OpenClaw 等环境时，平台可能将 **`CLAUDE_SKILL_DIR`** 设为技能根目录，即包含 `SKILL.md` 的目录。在本地用 Cursor 打开本仓库时，该目录即仓库根；`${CLAUDE_SKILL_DIR}/prompts/...` 与 `./prompts/...` 等价。
...
用户只有概念想法时可直接开始；如另有项目材料，可将 `examples/example_batch_job_scheduler/knowledge/` 的结构复制到自建案件目录（或 `outputs/{案件标识}/knowledge/`）后替换为真实原材料。
