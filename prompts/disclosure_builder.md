...
4. 三、本发明技术方案的详细阐述
   - 3.1 背景
   - 3.2 系统框图（可用 **mermaid** 作为源，如 `flowchart` + `subgraph` 分层；交付前转 PNG 或等价图片；**不要** ASCII 文字框图；定稿图须通过框线关系检查）
   - 3.3 模块功能说明（聚焦作用与关联关系，非输入输出）
   - 3.4 系统流程说明（可用 **mermaid 流程图** 作为源，交付前转 PNG 或等价图片；**不要** ASCII 文字/箭头流程图；定稿图须通过框线关系检查）
...
- **系统框图与流程图**可先用 **fenced mermaid** 描述；若 mermaid 自动布局导致框线重叠、线穿框、线压字、反馈线绕不开，须手工重绘或用脚本生成清晰 PNG，不能把有重叠的图作为定稿。
- **黑白图示硬标准**：定稿图片采用黑白框线风格，避免彩色填充、彩色线条、渐变、阴影和装饰性配色；可用线型、粗细、虚实或位置关系表达层级，不依赖颜色表达含义。
- **框线关系硬标准**：连接线不得穿过框体、节点文字、标题、菱形判断框；箭头应从框边缘进出；反馈线/回路线优先沿外侧空白区绕行；分支线与主线保持可辨识间距。
- **图片内容边界**：图中只放图标题、节点、连线和必要连线标签。对话中的解释、审稿备注、自检意见、如“说明：……”之类的说明性文字不得渲染到图片像素中；如需说明，写在正文图后普通段落。
...
- Markdown 中保留 LaTeX 源码；Word 版必须显示为公式编辑器/OMML 等原生公式对象，**不得**把公式渲染为 PNG 图片，也不得让用户打开 Word 后看到 `\theta`、`\mathrm`、`\[`、`\(` 等源码。
- 生成 Word 时优先使用 `tools/docx_omml_equations.py` 或等价流程：先用占位符生成 docx，再把公式替换为 OMML 对象。若只能得到图片公式或反斜杠源码，视为未完成定稿，须继续修复。
- 自检方法：解压 `.docx` 后 `word/document.xml` 中应出现 `<m:oMath`；不应出现 `\theta`、`\mathrm`、`\[`、`\(` 等公式源码；`word/media/` 中不应出现公式图片，只应包含系统图/流程图等必要插图。
...
1. **Markdown**：定稿 `.md` 可保留 `` ```mermaid`` 围栏源码或直接引用定稿 PNG；含公式时保留 LaTeX。
2. **Word**：对上一文件执行 `md_to_docx.py` 或 `docx_omml_equations.py`。含公式时必须用 `docx_omml_equations.py` 或等价流程生成原生公式对象；无公式时可使用 `mermaid_render.py` 默认 Word 流程。
