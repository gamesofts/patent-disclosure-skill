...
仅在用户提供项目路径、附件、设计文档、代码或已有交底书时执行。按优先级扫描并提取**可专利化**内容，用于校准 `concept_design.md` 形成的方案底稿。**根据当前项目结构调整扫描路径**。

若用户只输入概念想法且未提供材料，跳过本步骤，不要要求用户必须提供项目。
...
- 将扫描结果分成「已由材料证明的事实」与「仍来自概念推演的假设」，后续写交底书时不得混淆。
