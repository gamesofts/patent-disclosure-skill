...
- 定稿图须使用黑白框线风格，避免彩色填充、彩色线条、渐变、阴影或装饰性配色；同时检查框线关系：线不穿框、不压文字、不压标题、不穿菱形判断框；反馈线优先走外侧空白区。图片内只保留图标题、节点、连线和必要连线标签，不放对话说明或“说明：……”类文字。
...
- 若自动布局导致框线重叠，须调整布局或重绘图片后再生成 Word；不要把线穿框、线压字的图片作为定稿。
...
- **Word 原生公式**：最终 `.docx` 中公式应为 Word 公式编辑器/OMML 对象，不是公式图片，也不是可见 LaTeX 反斜杠源码。
