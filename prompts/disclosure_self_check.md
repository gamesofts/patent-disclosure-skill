...
- [ ] **Word 原生公式**：交付 `.docx` 中公式是否为 Word 公式编辑器/OMML 对象，而不是 PNG 图片或反斜杠源码？可用 `unzip -p xxx.docx word/document.xml` 检查：应有 `<m:oMath`，不应有 `\theta`、`\mathrm`、`\[`、`\(` 等源码；`word/media/` 不应包含公式图片。
...
- [ ] **系统框图与流程图**：定稿图已生成并嵌入 Word，**无** ASCII 文字流程图/框图；图片为黑白框线风格，无彩色填充、彩色线条、渐变、阴影或装饰性配色；框与线关系清晰，线不穿框、不压字、不压标题、不穿菱形判断框，反馈线优先走外侧空白区；图片中不含对话式说明文字、自检备注或“说明：……”类解释段。
