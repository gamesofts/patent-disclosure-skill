...
2. **分类**：
   ...
   - **符号与公式体例**：上标维度（如 `^{cpu}`）、符号多义、LaTeX 分隔符混用、3.5 与 3.4.1 不同形 → 改 **3.4.1 符号表**、相关公式、**3.5 符号列**及第六章实施例；遵循 **`disclosure_builder.md` §7.7** 与 **`template_reference.md` §3.4.1** 正/反例。若用户反馈 Word 中看到反斜杠源码或公式图片，必须重建 `.docx` 为 Word 原生公式对象（OMML/公式编辑器格式），不可仅改 Markdown。
   - **图示版式**：出现彩色填充/彩色线条、框线重叠、线穿框、反馈线压字、图片里出现对话说明/“说明：……”等 → 重新生成黑白框线图，确保图内仅有标题、节点、连线和必要连线标签，解释性文字写回正文而非图片像素。
   - **查新与区别**：现有技术或区别论述不准 → 改第一章，必要时再检索。
   - **保护点与表述**：第四章、第五章论点 → 与第三章对齐，避免矛盾。
3. **落地修改**：形成纠正后全文，**写入新文件** **`{案件名}_{YYYYMMDDHHmmss}.md`** 并生成同名 `.docx`（§7.3 第 5 点）；含公式时优先用 `docx_omml_equations.py` 或等价流程生成 Word 原生公式；**禁止**无必要大段重写无关章节；**勿默认覆盖**用户上一版文件名。
